...
/**
 * Registration page.  Renders a form for collecting an email and
 * password.  After successful registration the user is
 * automatically signed in and redirected to the chat interface.
 */
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Automatically log the user in using the returned data if available
      // Some backends return the user on registration without a token.  We'll
      // call the login endpoint to obtain the token.
      ...
      if (loginRes.ok) {
        ...
        // Fallback: registration succeeded but login failed for some reason
        setError(loginData.detail || 'Registered but unable to log in');
        setLoading(false);
      }
    } catch (err) {
