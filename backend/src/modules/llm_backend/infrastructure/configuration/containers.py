...
from ...application.generation.service import GenerationService
from ...application.tokenization.service import TokenizationService


class LLMDIContainer(containers.DeclarativeContainer):
    ...
    wiring_config = containers.WiringConfiguration(
        ...
        packages=["src.modules.llm_backend.application"],
    )
    ...
    # Placeholder dependencies until actual implementations are mapped
    responses_repository = providers.Dependency()
    tokens_repository = providers.Dependency()

    generation_service = providers.Factory(
        GenerationService,
        repository=responses_repository,
    )

    tokenization_service = providers.Factory(
        TokenizationService,
        repository=tokens_repository,
    )
